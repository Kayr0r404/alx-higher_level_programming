...
* [Properties vs. Getters and Setters (Mainly the last part “Public instead of Private Attributes”)](https://www.youtube.com/watch?v=rq8cL2XMM5M)
...
* Private instance attribute: `width`:
    * property d`ef width(self):` to retrieve it
    * property setter `def width(self, value):` to set it:
        * `width` must be an integer, otherwise raise a `TypeError` exception with the message `width must be an integer`
        * if `width` is less than `0`, raise a `ValueError` exception with the message `width must be >= 0`
* Private instance attribute: `height`:
...
rite a class `Rectangle` that defines a rectangle by: (based on `8-rectangle.py`)
